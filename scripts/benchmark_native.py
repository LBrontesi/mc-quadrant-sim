...
def benchmark(
    ...
    states = ["benchmark"]
    mean = pd.Series([0.006, 0.002, 0.003, 0.001], index=assets)
    ...
    correlation = pd.DataFrame(
        covariance.to_numpy() / (volatility[:, None] * volatility[None, :]),
        index=assets,
        columns=assets,
    )
    model = ScenarioModel(
        states=states,
        transition_matrix=pd.DataFrame([[1.0]], index=states, columns=states),
        moments={
            states[0]: RegimeMoments(
                mean=mean,
    ...
    report: dict[str, object] = {
        ...
        "target_under_4_5_seconds": baseline_seconds <= 4.5,
