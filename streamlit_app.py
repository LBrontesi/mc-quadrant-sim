...
def scenario_payload() -> dict:
    return {
        ...
        "cost_bps": 0 if st.session_state["rebalance"] == "legacy" else st.session_state["cost_bps"],
        "expense_ratios": st.session_state["expense_ratios"],
        "leverage_multiple": st.session_state["leverage_multiple"],
        "financing_rate": st.session_state["financing_rate"],
        "maintenance_margin": st.session_state["maintenance_margin"],
        "contribution": st.session_state["contribution"],
        "withdrawal": st.session_state["withdrawal"],
        "risk_free_rate": st.session_state["risk_free_rate"],
        "annual_inflation": st.session_state["annual_inflation"],
        ...
        "threshold_window": st.session_state["threshold_window"],
        "duration_model": st.session_state["duration_model"],
        "garch": st.session_state["garch"],
        "walk_forward": st.session_state["walk_forward"],
    }
...
st.sidebar.header("Model methodology")
model_kind = st.sidebar.selectbox(
    "Regime model",
    ["quadrant", "hmm"],
    format_func=lambda value: {
        "quadrant": "Quadrant (macro thresholds)",
        "hmm": "HMM (learned from returns)",
    }[value],
    help="Quadrant builds the four macro regimes from growth/inflation thresholds; "
    "HMM fits states directly on the asset returns.",
    key="model_kind",
)
st.sidebar.number_input(
    ...
    value=4,
    key="hmm_states",
    help="Number of hidden states for the HMM regime model.",
)
st.sidebar.selectbox(
    ...
    format_func=lambda value: {
        "markov": "Markov chain",
        "semi_markov": "Semi-Markov (empirical sojourns)",
    }[value],
    help="Semi-Markov draws regime run lengths from the empirical sojourn "
    "distribution instead of the geometric lengths of a first-order chain.",
    key="duration_model",
)
st.sidebar.number_input(
    "Causal threshold window",
    min_value=0,
    value=12,
    key="threshold_window",
    help="Prior observations required before classification; 0 uses the full sample (look-ahead bias).",
)
st.sidebar.checkbox(
    "GARCH(1,1) volatility clustering",
    value=False,
    key="garch",
    help="Within-regime conditional variance dynamics (Normal distribution only).",
)
st.sidebar.checkbox(
    "Walk-forward validation",
    value=True,
    key="walk_forward",
    help="Out-of-sample check of the regime model against an unconditional benchmark.",
)
if model_kind == "hmm":
    st.sidebar.info(
        "HMM states are learned from returns. Quadrant thresholds, macro lag, "
        "correlation overrides, and walk-forward validation do not affect HMM fitting."
    )
...
st.sidebar.number_input(
    ...
    value=10,
    key="cost_bps",
    disabled=st.session_state["rebalance"] == "legacy",
    help="Legacy mode has no modeled rebalancing event, so transaction costs are set to zero.",
)
if st.session_state["rebalance"] == "legacy":
    st.sidebar.caption("Legacy accounting selected: transaction costs are disabled.")
st.sidebar.text_input(
    "ETF expense ratios (ASSET:annual %)",
    key="expense_ratios",
    placeholder="SPY:0.03, IEF:0.15",
    help="Forward fee drag in annual percentage points. Historical ETF prices may already include fund fees.",
)
st.sidebar.number_input(
    "Leverage multiple",
    min_value=1.0,
    ...
    help="1.0x is unlevered. Leverage uses explicit borrowed balance and requires rebalancing mode.",
)
st.sidebar.number_input(
    ...
    step=0.1,
    key="financing_rate",
    help="Deterministic annual borrowing rate applied to the leveraged debt balance.",
)
st.sidebar.number_input(
    "Maintenance margin (%)",
    min_value=0.0,
    ...
    help="Set above zero to liquidate a path when equity/assets falls below this level.",
)
st.sidebar.number_input(
    "Contribution / period",
    min_value=0.0,
    ...
    step=10.0,
    key="contribution",
    help="Currency units invested at the target allocation each period (DCA).",
)
st.sidebar.number_input(
    "Withdrawal / period",
    min_value=0.0,
    ...
    step=10.0,
    key="withdrawal",
    help="Currency units funded pro-rata from holdings each period.",
)
...
st.sidebar.checkbox(
    "Blend custom correlation view",
    value=True,
    key="use_corr_override",
    disabled=model_kind == "hmm",
)
st.sidebar.slider(
    "Blend",
    0.0,
...
for name in REGIME_ORDER_NAMES:
    default = api.DEFAULT_CORRELATIONS[
        next(state for state in api.REGIME_ORDER if api.REGIME_NAMES[state] == name)
    ]
    st.sidebar.slider(
        name,
        -1.0,
...
with st.spinner("Loading data..."):
    try:
        ...
        st.session_state["load_response"] = load
...
with st.expander("Data coverage and previews", expanded=False):
    load_response = st.session_state["load_response"]
    coverage = pd.DataFrame.from_dict(load_response["coverage"], orient="index")
    if not coverage.empty:
        coverage.index.name = "Asset"
        st.dataframe(coverage, width="stretch")
    st.caption(load_response["message"])
    preview_tabs = st.tabs(["Macro preview", "Returns preview"])
    for tab, key in zip(preview_tabs, ("macro", "returns")):
        with tab:
            preview = load_response[key]
            st.dataframe(
...
current_payload = sim_payload(selected, weights) if selected else None
configuration_error = bool(
    current_payload
    and current_payload.get("garch")
    and current_payload.get("distribution") != "normal"
)
if current_payload and current_payload.get("leverage_multiple", 1.0) > 1.0 and current_payload.get("rebalance") == "legacy":
    configuration_error = True
    st.error("Leverage requires monthly, quarterly, or annual rebalancing; legacy accounting is not supported.")
if configuration_error:
    if current_payload and current_payload.get("garch") and current_payload.get("distribution") != "normal":
        st.error("GARCH volatility clustering requires the Normal return distribution.")

last_payload = st.session_state.get("last_sim_payload")
stale_results = bool(
    current_payload
    and last_payload
    and _payload_signature(current_payload) != _payload_signature(last_payload)
)
if stale_results:
    st.warning("Inputs changed since the displayed simulation. Run again before interpreting or exporting results.")

run = st.button(
    "Run Simulation",
    type="primary",
    disabled=not selected or sum(weights.values()) <= 0 or configuration_error,
)
...
if validation:
    ...
    st.caption(
        f"Out-of-sample validation: regime model "
...
for index, (key, label) in enumerate(SUMMARY_METRICS):
    ...
    metric_cols[index % 5].metric(
        label,
        api.format_metric_value(key, value, results["currency"]),
    )

if summary.get("periodic_contribution", 0) or summary.get("periodic_withdrawal", 0):
    st.subheader("Investment performance excluding external cash flows")
    st.caption(
        "Time-weighted metrics remove the effect of regular contributions and withdrawals. "
        "Terminal wealth metrics above still include those cash flows."
    )
    ...
    for index, (key, label) in enumerate(flow_metrics):
        flow_cols[index].metric(
            label,
            api.format_metric_value(key, summary.get(key), results["currency"]),
        )

costs = results.get("costs", {})
if costs.get("leverage_multiple", 1.0) > 1.0 or costs.get("weighted_expense_ratio", 0.0) > 0:
    st.subheader("Cost and leverage assumptions")
    st.caption(
        "Expense ratios are modeled as forward monthly fee drag. ETF price histories may already include historical fees."
    )
    cost_cols = st.columns(5)
    for index, (key, label) in enumerate(
        [
            ("leverage_multiple", "Leverage"),
            ("weighted_expense_ratio", "Weighted ETF fee"),
            ("annual_fee_drag", "Annual fee drag"),
            ("annual_financing_cost", "Annual financing cost"),
            ("margin_calls", "Margin calls"),
        ]
    ):
        value = costs.get(key, 0.0)
        formatted = f"{value:.1f}x" if key == "leverage_multiple" else api.format_metric_value(key, value, results["currency"])
        cost_cols[index].metric(label, formatted)
...
with chart_row2_right:
    ...
    regime_labels = list(regime_mix["label"])
    st.altair_chart(
        alt.Chart(regime_mix)
        .mark_bar()
        .encode(
            x=alt.X("label:N", title=None, sort=regime_labels),
            y=alt.Y("share:Q", title="Share"),
            color=alt.Color(
                "label:N",
                scale=alt.Scale(
                    domain=regime_labels,
                    range=[REGIME_COLORS.get(label, "#64748b") for label in regime_labels],
                ),
                legend=None,
            ),
        )
...
with chart_row3_left:
    ...
    scatter_labels = list(scatter["regime"].dropna().unique())
    st.altair_chart(
        alt.Chart(scatter)
        ...
        .encode(
            ...
            color=alt.Color(
                "regime:N",
                scale=alt.Scale(
                    domain=scatter_labels,
                    range=[REGIME_COLORS.get(label, "#64748b") for label in scatter_labels],
                ),
            ),
...
with chart_row3_right:
    ...
    transition_labels = list(transition["labels"])
    matrix_rows = [
    ...
    st.altair_chart(
        ...
        .mark_rect()
        .encode(
            x=alt.X("To:N", sort=transition_labels),
            y=alt.Y("From:N", sort=transition_labels),
...
st.download_button(
    "Risk summary (CSV)",
    pd.DataFrame(summary_rows, columns=["metric", "value"]).to_csv(index=False),
    "risk_summary.csv",
    "text/csv",
)
st.download_button(
    "Diagnostics (CSV)", diagnostics.to_csv(index=False), "calibration_diagnostics.csv", "text/csv"
)
